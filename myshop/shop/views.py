...
#popuse of here in optain and showing products, so for that we have two way: one, user send us category and we easy query and optain that product(like shop/categories/samsung/) and showing that to user. second if category=None and user dont send eny category?? how we find that products? it post to use so user like url like  shop/categories/
#<form> here in like: <form action="{% url 'shop:product_list_by_category' %}" method="get">    form without action wil default to current url and default method is get.
