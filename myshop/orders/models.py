...
class Order:         
    ...
    update
    paid
#instead of saving for every orderitem, a seperate order, we just save one Order for group of order items.
#so in Order, instead of saving address, name, code.... of Order in database in every Order object created, (that this fields is same in every Order) we implement this fields on OrderProfle so because of that,    address, name, postal_code .... just saved in database in one(for example <OrderProfile (1)>) and for every order we created for example Order1, Order2, Order4.. we just referec to OrderProfile1 and so we just create one time this fields(address, name..) in multiple Orders.
